...
pub(crate) fn init_ollama() -> anyhow::Result<BaseProvider> {
    let name = ask_name("Specify Ollama provider name")?;
    let endpoint_url =
        ask_endpoint_url("Specify Ollama endpoint URL", Some("https://ollama.com/v1"))?;
    ...
    let provider = BaseProvider::new(name, endpoint_url, token_key, model, proxy_scheme);
